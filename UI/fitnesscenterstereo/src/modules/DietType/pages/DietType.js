...
@inject(
    i => ({
        dietTypeViewStore: new DietTypeViewStore(i.rootStore)
    })
)
@observer
class DietType extends React.Component {
    render() {
        const { searchQuery, isLoading, columns, isDeleting, page, recordsPerPage, displayItems, itemToDeleteName, onUpdate, onCreate, onFind, onDelete, onSearchQueryChange, onRecordsPerPageChange, onSortChange, onPageChange, onConfirmDelete, onCancelDelete } = this.props.dietTypeViewStore;
        return (
            <React.Fragment>
                <Layout>
